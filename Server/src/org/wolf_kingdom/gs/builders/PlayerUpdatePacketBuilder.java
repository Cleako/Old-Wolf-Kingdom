...
import org.wolf_kingdom.gs.model.Projectile;
	...
	public RSCPacket getPacket() {
		...
		if (updateSize > 0) {
			...
			for (Projectile p : projectilesNeedingDisplayed) { // 3/4 - Draws a projectile
				...
				Entity victim2 = p.getCannon();
				if (victim instanceof Npc) { //Player Casting at an NPC
					...
					System.out.println("Cast on NPC");
				}
				else if (victim instanceof Player) {  //Player Casting on another Player
					updates.addShort(p.getCaster().getIndex());
					updates.addByte((byte) 4);
					updates.addShort(p.getType());
					updates.addShort(((Player) victim).getIndex());
					System.out.println("Cast on Player");
				}
				else if(victim2 instanceof Npc) { //Object Casting at Npc?
					updates.addShort(p.getCannon().getIndex());
					updates.addByte((byte)3);
					updates.addShort(p.getType());
					updates.addShort(((GameObject) victim2).getIndex());
