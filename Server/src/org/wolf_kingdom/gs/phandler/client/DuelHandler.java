...
import org.wolf_kingdom.gs.event.DuelEvent;
...
public class DuelHandler implements PacketHandler {
	...
	if (player == affectedPlayer) {
		Logger.println("Warning : " + player.getUsername() + " tried to duel himself");
	    unsetOptions(player);
	...
	case 222: // Sending duel request
		...
		world.addEntryToSnapshots(new Activity(player.getUsername(), player.getUsername() + " sent duel request " + affectedPlayer.getUsername() + " at: " + player.getX() + "/" + player.getY() + " | " + affectedPlayer.getX() + "/" + affectedPlayer.getY()));
	    }
	    break;
	case 252: // Duel accepted
	    affectedPlayer = player.getWishToDuel();
	    if (affectedPlayer == null || busy(affectedPlayer) || !player.isDueling() || !affectedPlayer.isDueling()) { // This
		// shouldn't
		// happen
	    ...
	    world.addEntryToSnapshots(new Activity(player.getUsername(), player.getUsername() + " accepted duel request " + affectedPlayer.getUsername() + " at: " + player.getX() + "/" + player.getY() + " | " + affectedPlayer.getX() + "/" + affectedPlayer.getY()));

	    break;
	case 87: // Confirm accepted
	    affectedPlayer = player.getWishToDuel();
	    if (affectedPlayer == null || busy(affectedPlayer) || !player.isDueling() || !affectedPlayer.isDueling() || !player.isDuelOfferAccepted() || !affectedPlayer.isDuelOfferAccepted()) { // This
		// shouldn't
		// happen
		player.setSuspiciousPlayer(true);
		unsetOptions(player);
	    ...
	    if (affectedPlayer.isDuelConfirmAccepted()) {
			if(PluginHandler.getPluginHandler().blockDefaultAction("Duel", new Object[] { player, affectedPlayer })) {
				return;
			}
			...
			WalkToMobEvent walking = new WalkToMobEvent(player, affectedPlayer, 1) {
			    public void arrived() {
			    	World.getWorld().getDelayedEventHandler().add(new SingleEvent(owner, 300) {
				    public void action() {
					...
					World.getWorld().getDelayedEventHandler().add(dueling);
				    }
				});
			    }
			...
			World.getWorld().getDelayedEventHandler().add(walking);
		    }
	    break;
	case 35: // Decline duel
	    ...
	    unsetOptions(player);
	    unsetOptions(affectedPlayer);
		world.addEntryToSnapshots(new Activity(player.getUsername(), player.getUsername() + " declined duel " + affectedPlayer.getUsername() + " at: " + player.getX() + "/" + player.getY() + " | " + affectedPlayer.getX() + "/" + affectedPlayer.getY()));

	    break;
	case 123: // Receive offered item data
	    affectedPlayer = player.getWishToDuel();
	    if (affectedPlayer == null || busy(affectedPlayer) || !player.isDueling() || !affectedPlayer.isDueling() || (player.isDuelOfferAccepted() && affectedPlayer.isDuelOfferAccepted()) || player.isDuelConfirmAccepted() || affectedPlayer.isDuelConfirmAccepted()) { // This
		// shouldn't
		// happen
		player.setSuspiciousPlayer(true);
		unsetOptions(player);
