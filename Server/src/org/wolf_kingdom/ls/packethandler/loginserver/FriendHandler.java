...
public class FriendHandler implements PacketHandler {
	...
	case 11: // Add friend
		...
		Server.db.updateQuery("INSERT INTO `wk_friends`(`user`, `friend`) VALUES('" + user + "', '" + friend + "')");
		if (Server.db.getQuery("SELECT 1 FROM `wk_players` AS p LEFT JOIN `wk_friends` AS f ON f.user=p.user WHERE (p.block_private=0 OR f.friend='" + user + "') AND p.user='" + friend + "'").next()) {
		    w = server.findWorld(friend);
		    if (w != null) {
			world.getActionSender().friendLogin(user, friend, w.getID());
		    }
		}
		if (Server.db.getQuery("SELECT 1 FROM `wk_players` AS p LEFT JOIN `wk_friends` AS f ON f.friend=p.user WHERE p.block_private=1 AND f.user='" + friend + "' AND p.user='" + user + "'").next()) {
		    w = server.findWorld(friend);
		    if (w != null) {
	...
	case 12: // Remove friend
		...
		Server.db.updateQuery("DELETE FROM `wk_friends` WHERE `user` LIKE '" + user + "' AND `friend` LIKE '" + friend + "'");
		if (Server.db.getQuery("SELECT 1 FROM `wk_players` WHERE block_private=1 AND user='" + user + "'").next()) {
		    w = server.findWorld(friend);
		    if (w != null) {
	...
	case 13: // Add ignore
		...
		Server.db.updateQuery("INSERT INTO `wk_ignores`(`user`, `ignore`) VALUES('" + user + "', '" + friend + "')");
	    } catch (SQLException e) {
		Server.error(e.getMessage());
	    }
	    break;
	case 14: // Remove ignore
		...
		Server.db.updateQuery("DELETE FROM `wk_ignores` WHERE `user` LIKE '" + user + "' AND `ignore` LIKE '" + friend + "'");
	    } catch (SQLException e) {
		Server.error(e.getMessage());
	    }
	    break;
