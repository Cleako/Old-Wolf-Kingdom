...
import org.wolf_kingdom.ls.packetbuilder.loginserver.PlayerLoginPacketBuilder;
...
public class PlayerLoginHandler implements PacketHandler {
	...
	if (loginCode == 0 || loginCode == 1 || loginCode == 99) {
	    ...
	    try {
		Server.db.updateQuery("UPDATE `wk_players` SET online=1 WHERE user='" + user + "'");
	    } catch (Exception e) {
	    }
	...
	try {
		ResultSet result = Server.db.getQuery("SELECT r.pass, r.banned, r.phpbb_id, u.user_id, r.group_id, b.ban_userid AS ban_userid FROM `wk_players` AS r INNER JOIN `phpbb_users` AS u ON u.user_id=r.phpbb_id LEFT JOIN `phpbb_banlist` AS b on (b.ban_userid LIKE u.user_id OR b.ban_ip LIKE '" + ip + "') WHERE `user`='" + user + "'");
	    if (!result.next()) {
	    	return 10;
	    }
