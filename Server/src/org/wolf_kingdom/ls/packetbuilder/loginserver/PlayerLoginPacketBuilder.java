...
import org.wolf_kingdom.ls.model.BankItem;
import org.wolf_kingdom.ls.model.InvItem;
...
public class PlayerLoginPacketBuilder {
    ...
    public LSPacket getPacket() {
		...
		if (save != null) {
		    ...
		    try {
				ResultSet result = Server.db.getQuery("SELECT p.user FROM `wk_friends` AS f INNER JOIN `wk_players` AS p ON p.user=f.friend WHERE p.block_private=0 AND f.user='" + save.getUser() + "'");
				while (result.next()) {
				    friendsWithUs.add(result.getLong("user"));
				}
				result = Server.db.getQuery("SELECT user FROM `wk_friends` WHERE friend='" + save.getUser() + "'");
				while (result.next()) {
				    friendsWithUs.add(result.getLong("user"));
				}
		    } 
