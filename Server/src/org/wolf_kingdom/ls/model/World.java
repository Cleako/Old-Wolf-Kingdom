...
import org.wolf_kingdom.ls.packetbuilder.loginserver.MiscPacketBuilder;
...
public class World {
	...
	ResultSet result;
	try {
	    result = Server.db.getQuery("SELECT owner, block_private FROM `wk_players` WHERE `user`='" + user + "'");
	    if (!result.next()) {
		return;
	    }
	    ...
	    result = Server.db.getQuery("SELECT user FROM `wk_friends` WHERE `friend`='" + user + "'" + (blockPrivate ? " AND user IN (SELECT friend FROM `wk_friends` WHERE `user`='" + user + "')" : ""));
	    while (result.next()) {
	    ...
	    Server.db.updateQuery("INSERT INTO `wk_logins`(`user`, `time`, `ip`) VALUES('" + user + "', '" + now + "', '" + ip + "')");
	    Server.db.updateQuery("UPDATE `wk_players` SET login_date=" + now + ", login_ip='" + ip + "' WHERE user='" + user + "'");
	...
	try {
	    Server.db.updateQuery("UPDATE `wk_players` SET online=0 WHERE user='" + user + "'");
	} catch (Exception e) {
	    Server.error(e);
	}
