...
import org.lupus_regnum.gs.plugins.minigames.cannons.CannonAssembling;
...
public class InvActionHandler implements PacketHandler {
    ...
    public void handlePacket(Packet p, IoSession session) throws Exception {
		...
		if (item.isEdible()) {
			...
			break;
			case 566: // cure poision potion - 3 dose
			usePoisonPotion(player, item, 567, 2);
			break;
			case 567: //cure poison potion - 2 dose
			usePoisonPotion(player, item, 568, 1);
			break;
			case 568: //cure posion potion - 1 dose
			usePoisonPotion(player, item, 465, 0);
			break;
		    default:
    ...
    private void useFishingPotion(Player player, final InvItem item, final int newItem, final int left) {
		...
		World.getWorld().getDelayedEventHandler().add(new MiniEvent(player) {
		    public void action() {
				...
				owner.getInventory().remove(item); // Get rid of it!
			if(newItem == 465) { // Empty vial
				owner.getActionSender().sendInventory();
                owner.setBusy(false);
			} 
			else {
				owner.getInventory().add(new InvItem(newItem)); //Give them their lower dose potion
                owner.getActionSender().sendInventory();
                owner.setBusy(false);
                }
		    }
	...
	private void usePoisonPotion(Player player, final InvItem item, final int newItem, final int left) {
		player.setBusy(true);
		player.getActionSender().sendMessage("You drink some of your " + item.getDef().getName() + ".");
		World.getWorld().getDelayedEventHandler().add(new MiniEvent(player) {
		    public void action() {
			...
			if(owner.cured = true) {
			owner.getActionSender().sendMessage("You have been cured.");
			}
			if(owner.poisoned = false) {
			owner.getActionSender().sendMessage("You are still poisoned.");
			}
			owner.getInventory().remove(item); // Get rid of it!
			if(newItem == 465) { // Empty vial
				owner.getActionSender().sendInventory();
