...
<?php if ($this->_rootref['S_HAS_POLL']) {  ?>
	...
	<hr />
<?php } $_postrow_count = (isset($this->_tpldata['postrow'])) ? sizeof($this->_tpldata['postrow']) : 0;if ($_postrow_count) {for ($_postrow_i = 0; $_postrow_i < $_postrow_count; ++$_postrow_i){$_postrow_val = &$this->_tpldata['postrow'][$_postrow_i]; if ($this->_tpldata['DEFINE']['.']['PROFILE_SIDE'] == ('left')) {  ?><div class="profilesleft"><?php } if ($_postrow_val['S_FIRST_UNREAD']) {  ?><a id="unread"></a><?php } ?>
	<div id="p<?php echo $_postrow_val['POST_ID']; ?>" class="post <?php if (($_postrow_val['S_ROW_COUNT'] & 1)  ) {  ?>bg1<?php } else { ?>bg2<?php } if ($_postrow_val['S_UNREAD_POST']) {  ?> unreadpost<?php } if ($_postrow_val['S_POST_REPORTED']) {  ?> reported<?php } if ($_postrow_val['S_ONLINE'] && ! $_postrow_val['S_IGNORE_POST'] && $this->_tpldata['DEFINE']['.']['PROFILE_SIDE'] != ('left')) {  ?> online<?php } ?>">
		<div class="inner"><span class="corners-top"><span></span></span>
		...
		<?php if (! $_postrow_val['S_IGNORE_POST']) {  ?>
			<dl class="postprofile<?php if ($_postrow_val['S_ONLINE'] && ! $_postrow_val['S_IGNORE_POST'] && $this->_tpldata['DEFINE']['.']['PROFILE_SIDE'] == ('left')) {  ?> online<?php } ?>" id="profile<?php echo $_postrow_val['POST_ID']; ?>">
			<dt>
                ...
                <?php echo $_postrow_val['POSTER_AVATAR_IMG_SRC']; ?>
			</dt>

			<?php if ($_postrow_val['RANK_TITLE'] || $_postrow_val['RANK_IMG']) {  ?><dd><?php echo $_postrow_val['RANK_TITLE']; if ($_postrow_val['RANK_TITLE'] && $_postrow_val['RANK_IMG']) {  ?><br /><?php } echo $_postrow_val['RANK_IMG']; ?></dd><?php } if ($_postrow_val['EXTRA_RANK_TITLE'] || $_postrow_val['EXTRA_RANK_IMG']) {  ?><dd><?php echo $_postrow_val['EXTRA_RANK_TITLE']; if ($_postrow_val['EXTRA_RANK_TITLE'] && $_postrow_val['EXTRA_RANK_IMG']) {  ?><br /><?php } echo $_postrow_val['EXTRA_RANK_IMG']; ?></dd><?php } ?>
	...
	<hr class="divider" />
<?php if ($this->_tpldata['DEFINE']['.']['PROFILE_SIDE'] == ('left')) {  ?></div><?php } }} if ($this->_rootref['S_QUICK_REPLY']) {  $this->_tpl_include('quickreply_editor.html'); } if ($this->_rootref['S_NUM_POSTS'] > (1) || $this->_rootref['PREVIOUS_PAGE']) {  ?>
