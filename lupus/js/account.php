...
if($_POST['nm']){
	...
	$user_result = $connector->query("SELECT user FROM rsca2_players WHERE username='$username'");
	...
	if($num_users_row != 0){
		...
		if(strlen($username) >= 12 || strlen($username) <= 4 || strlen($password) <= 5){} else {
			...
			$connector->query("INSERT INTO `rsca2_curstats`(`user`) VALUES ('" . $gamename[0] . "')");
			$connector->query("INSERT INTO `rsca2_experience`(`user`) VALUES ('" . $gamename[0] . "')");
			$connector->query("INSERT INTO `rsca2_players`(`user`, `username`, `phpbb_id`, `owner_username`, `pass`, `creation_date`, `creation_ip`) VALUES ('" . $gamename[0] . "', '" . $username . "', '" . $user->data['user_id'] . "', '" . $user->data['username'] . "', '" . $gamepass . "', '".$time."', '". $_SERVER['REMOTE_ADDR'] ."')");
			echo 1;
		}
	}
	...
	if(strtolower($ver) != 'yes'){
		...
		$user_check = $connector->query("SELECT phpbb_id,group_id FROM rsca2_players WHERE user=$user_ui");
		$check = $connector->fetchArray($user_check);
		if($user_i == $user->data['user_id']){
			if($check['group_id'] == 1){ echo 2; } else {
				$connector->query("DELETE FROM rsca2_players WHERE user = '" . $user_ui . "'");
				$connector->query("DELETE FROM rsca2_curstats WHERE user = '" . $user_ui . "'");
				$connector->query("DELETE FROM rsca2_experience WHERE user = '" .  $user_ui . "'");
				$connector->query("DELETE FROM rsca2_invitems WHERE user = '" .  $user_ui. "'");
				$connector->query("DELETE FROM rsca2_quests WHERE user = '" .  $user_ui . "'");
				$connector->query("DELETE FROM rsca2_friends WHERE user = '" .  $user_ui. "'");
				$connector->query("DELETE FROM rsca2_ignores WHERE user = '" .  $user_ui . "'");
				$connector->query("DELETE FROM rsca2_bank WHERE user = '" .  $user_ui . "'");
				$connector->query("DELETE FROM rsca2_kills WHERE user = '" .  $user_ui . "'");
				echo 3;
			}
		} else { echo 1; }
	}
...
$user_check = $connector->query("SELECT ".$skills.",rsca2_players.phpbb_id FROM rsca2_players LEFT JOIN rsca2_experience ON rsca2_players.user = rsca2_experience.user WHERE rsca2_players.user=$userhash");
$check = $connector->fetchArray($user_check);
